...
The status page treats the offering contract as the source of truth for offering state. When a wallet/provider is available, the browser reads `unitsSold`, `remainingUnits`, `raised`, `raiseMin`, `closeDate`, `minMet`, `state`, `withdrawn`, `owner`, and `treasury` from the `Offering`, then stores that snapshot on the local issuance record. The page uses that snapshot for raised amount, tokens sold, threshold status, current valuation, proceeds, and the funded progress segment. Pending allocations remain local workflow records until purchased. The separate cap table section treats the Liquid Split as the source of truth for ownership: it reads current holder balances through the local `/api/liquid-splits/:address/holders` proxy, which calls the Splits Explorer public GraphQL API by default. If that indexed read fails, the browser falls back to direct Base RPC reads against `TransferSingle` logs plus current `balanceOf(holder, 0)` values. Older local issuances without onchain fields still render a target cap table from the saved issuance data.

Lifecycle actions are exposed from the status and buyer pages. `withdraw()` is permissionless once proceeds are claimable and always pays the treasury. `closeAndWithdraw()` is owner-only and returns unsold Liquid Split units to the treasury while withdrawing claimable proceeds. `markFailed()` is permissionless after the close date only if the minimum was never met. Buyer pages expose `refund()` for failed offerings when the connected wallet has a refundable USDC deposit; refunds return money only and do not move or burn Liquid Split units.
...
1. **Manual lifecycle testing on Base** — create tiny successful and failed offerings with dust USDC, then verify the status chips, claimable/withdrawn proceeds, owner-only close button, permissionless withdraw, permissionless failed marking, and buyer refund copy/action states against real wallet prompts.
2. **Production deployment hardening** — create the Fly.io app and persistent SQLite volume, set `PACT_DB_PATH=/data/pact.sqlite`, configure `SPLITS_EXPLORER_API_KEY`, choose the production region/app name, and document the exact Fly secrets/env setup after the account exists.
3. **Round-state polish** — after real lifecycle testing, revisit whether the secured/failed/closed visual treatment is obvious enough. The current branch makes these explicit chips, but the final UI may need stronger hierarchy once tested with real data.
